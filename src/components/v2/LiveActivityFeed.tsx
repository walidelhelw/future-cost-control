...
export function LiveActivityFeed({ locale }: LiveActivityFeedProps) {
  return (
    <aside className="rounded-md border border-white/10 bg-black/35 p-4 backdrop-blur-xl">
        ...
        {activityItems.map((item, index) => (
          ...
          >
            <div className="flex items-start gap-3">
              <CircleDot className={`mt-0.5 h-4 w-4 ${toneClass[item.tone] ?? "bg-cyan-300"} rounded-full text-black`} />
